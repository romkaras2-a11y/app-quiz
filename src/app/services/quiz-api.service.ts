 ...
 private proxyUrl = '/api-quiz/api/v1';
  ...
  async getQuizzes(): Promise<Quiz[]> {
    try {
      ...
      const apiResponse: any = await firstValueFrom(this.http.get(`${this.proxyUrl}/quizzes?tags=javascript,typescript,node&limit=2`));    
      ...
      const mappedApiQuizzes: Quiz[] = apiQuizzesArray.map((q: any) => ({
        ...
        title: `${q.title || q.name} (QuizAPI Live)`, // QuizAPI nutzt meist "title"
  ...
  private async fetchOnlineQuizQuestions(quizId: string): Promise<Quiz> {
    try {
      const url = `${this.proxyUrl}/questions?quiz_id=${quizId}&include_answers=true`;
